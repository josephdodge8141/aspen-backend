...
@router.delete(
    "/{service_id}/segments/{segment_id}", status_code=status.HTTP_204_NO_CONTENT
)
...
def link_service_to_experts(
    ...
    all_links_statement = select(ExpertService).where(
        ExpertService.service_id == service_id
    )
...
@router.delete(
    "/{service_id}/experts/{expert_id}", status_code=status.HTTP_204_NO_CONTENT
)
...
def link_service_to_workflows(
    ...
    for workflow_id in request.workflow_ids:
        if workflow_id not in existing_workflow_ids_linked:
            new_links.append(
                WorkflowService(workflow_id=workflow_id, service_id=service_id)
            )
    ...
    all_links_statement = select(WorkflowService).where(
        WorkflowService.service_id == service_id
    )
    all_links = session.exec(all_links_statement).all()
...
@router.delete(
    "/{service_id}/workflows/{workflow_id}", status_code=status.HTTP_204_NO_CONTENT
)
...
def get_service_exposure(
    ...
    return ExposureResponse(
        ...
        workflows=[
            ExposureItem(id=workflow.id, name=workflow.name) for workflow in workflows
        ],
        counts={
            "experts": len(experts),
...
def whoami(caller: CallerContext = Depends(get_caller)):
    if caller.service:
        ...
        statement = select(ServiceSegment).where(
            ServiceSegment.service_id == caller.service.id
        )
        segments = session.exec(statement).all()
