...
def validate_common_fields(metadata: Dict[str, Any], structured_output: Dict[str, Any]) -> None:
...
def validate_no_unknown_fields(metadata: Dict[str, Any], allowed_fields: set) -> None:
    ...
    if unknown_fields:
        raise NodeValidationError(f"Unknown fields in metadata: {', '.join(sorted(unknown_fields))}")


def validate_required_fields(metadata: Dict[str, Any], required_fields: set) -> None:
    missing_fields = required_fields - set(metadata.keys())
    if missing_fields:
        raise NodeValidationError(f"Missing required fields: {', '.join(sorted(missing_fields))}")
