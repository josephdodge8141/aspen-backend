...
class ServicesRepo:
    ...
    def get_by_name_and_env(
        self, session: Session, name: str, environment: Environment
    ) -> Optional[Service]:
    ...
    def list(
        self, session: Session, *, environment: Optional[Environment] = None
    ) -> List[Service]:
    ...
    def create_segment(
        self, session: Session, segment: ServiceSegment
    ) -> ServiceSegment:
        session.add(segment)
        session.commit()
    ...
    def get_segment(
        self, session: Session, segment_id: int
    ) -> Optional[ServiceSegment]:
        return session.get(ServiceSegment, segment_id)

    def get_segment_by_name(
        self, session: Session, service_id: int, name: str
    ) -> Optional[ServiceSegment]:
        statement = select(ServiceSegment).where(
            ServiceSegment.service_id == service_id, ServiceSegment.name == name
        )
        return session.exec(statement).first()
