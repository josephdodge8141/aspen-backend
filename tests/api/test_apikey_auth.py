...
def test_valid_api_key(db_session: Session, auth_client: TestClient):
    ...
    async def test_endpoint(caller: CallerContext = Depends(get_caller)):
        if caller.service:
            return {"service_id": caller.service.id, "service_name": caller.service.name}
...
def test_valid_jwt_token(db_session: Session, auth_client: TestClient):
    ...
    response = auth_client.get("/test-api-key", headers={"Authorization": f"Bearer {token}"})
...
def test_invalid_jwt_token(auth_client: TestClient):
    ...
    response = auth_client.get("/test-api-key", headers={"Authorization": "Bearer invalid-token"})
    
    assert response.status_code == 401
    assert response.headers.get("Content-Type") == "application/problem+json"
    assert "Invalid token" in response.json()["detail"] or "JWT_SECRET" in response.json()["detail"]


def test_malformed_authorization_header(auth_client: TestClient):
    # Test with malformed Authorization header
    response = auth_client.get("/test-api-key", headers={"Authorization": "NotBearer token"})
...
def test_both_api_key_and_jwt_prefers_jwt(db_session: Session, auth_client: TestClient):
    ...
    response = auth_client.get("/test-api-key", headers={
