...
def test_segment_key_json_storage(db_session):
    ...
    assert retrieved_service_user.segment_key["properties"]["metadata"]["plan"] == "premium"
    assert retrieved_service_user.segment_key["properties"]["metadata"]["features"] == ["feature_a", "feature_b"]
...
def test_foreign_key_integrity(db_session):
    ...
    member = Member(
        first_name="FK",
        last_name="Test",
        email=f"fk.test.{uuid.uuid4()}@example.com"
    )
    db_session.add(member)
    db_session.commit()
