...
def test_expert_with_different_statuses(db_session):
    ...
    statuses = [
        ExpertStatus.draft,
        ExpertStatus.active,
        ExpertStatus.archive,
        ExpertStatus.historical,
    ]
...
def test_unique_expert_service_constraint(db_session):
    ...
    expert_service1 = ExpertService(expert_id=expert.id, service_id=2)
    db_session.add(expert_service1)
    db_session.commit()

    # Try to create duplicate relationship
    expert_service2 = ExpertService(
        expert_id=expert.id, service_id=2  # Same service ID
    )
...
def test_unique_expert_workflow_constraint(db_session):
    ...
    expert_workflow1 = ExpertWorkflow(expert_id=expert.id, workflow_id=3)
    db_session.add(expert_workflow1)
    db_session.commit()

    # Try to create duplicate relationship
    expert_workflow2 = ExpertWorkflow(
        expert_id=expert.id, workflow_id=3  # Same workflow ID
    )
