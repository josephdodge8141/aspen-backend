...
def test_migration_status():
    ...
    result = subprocess.run(
        ["alembic", "current"], capture_output=True, text=True, cwd=os.getcwd()
    )
...
def test_migration_history():
    ...
    result = subprocess.run(
        ["alembic", "history"], capture_output=True, text=True, cwd=os.getcwd()
    )
...
def test_enum_types():
    ...
    with Session(engine) as session:
        ...
        enum_types = ["teamrole", "expertstatus", "environment", "nodetype"]
