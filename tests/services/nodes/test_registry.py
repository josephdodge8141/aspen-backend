...
class TestNodeServiceRegistry:
    ...
    def test_get_unregistered_service_raises_error(self):
        with pytest.raises(ValueError, match="No service registered for node type: NodeType.job"):
...
class TestDefaultNodeService:
    def test_default_service_validation_raises_error(self):
        ...
        with pytest.raises(NodeValidationError, match="Node service not implemented - validation not available"):
            service.validate({}, {})

    def test_default_service_plan_raises_error(self):
        service = DefaultNodeService()
        
        with pytest.raises(NodeExecutionError, match="Node service not implemented - planning not available"):
            service.plan({}, {}, {})

    def test_default_service_execute_raises_error(self):
        service = DefaultNodeService()
        
        with pytest.raises(NodeExecutionError, match="Node service not implemented - execution not available"):
            service.execute({}, {}) 