...
class TestRenderPrompt:
    ...
    def test_mixed_substitutions(self):
        template = "User {{ base.username }} wants to {{ input.action }} {{ input.target }}"
    ...
    def test_array_access(self):
        ...
        assert warnings == [] or len(warnings) <= 2  # May have warnings if JSONata doesn't support this syntax
    ...
    def test_fallback_to_input_then_base(self):
        ...
        assert "42" in result
        assert warnings == [] or len(warnings) <= 1  # Might have warning if 'name' not found in input
...
class TestGetBaseDefaults:
    def test_base_defaults_structure(self):
        ...
        expected_keys = {
            "timestamp", "date", "time", "timezone", "unix_timestamp",
