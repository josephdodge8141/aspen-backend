...
class TestToRead:
    ...
    def test_to_read_basic(self, sample_expert):
        ...
        assert result.input_params == {"param1": "value1", "param2": {"nested": "value"}}
        assert result.status == ExpertStatus.active
        assert result.model_name == "gpt-4"
        assert result.team_id == 10
...
class TestMapperIntegration:
    ...
    def sample_expert(self):
        ...
        return Expert(
            ...
            uuid="test-uuid-123",
            name="Test Expert",
            prompt="This is a moderately long prompt that might need truncation in list view but should be preserved in detail view. " + "A" * 50,  # Make it longer than 120 chars
            input_params={"param1": "value1"},
            status=ExpertStatus.active,
            model_name="gpt-4",
            team_id=10
