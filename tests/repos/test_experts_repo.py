...
def test_list_with_counts_no_filters(db_session: Session, experts_repo: ExpertsRepo, seed_data):
    ...
    expert1_result = next((r for r in results if r.name == seed_data["experts"][0].name), None)
    expert2_result = next((r for r in results if r.name == seed_data["experts"][1].name), None)
    expert3_result = next((r for r in results if r.name == seed_data["experts"][2].name), None)
    expert4_result = next((r for r in results if r.name == seed_data["experts"][3].name), None)
...
def test_list_with_counts_team_filter(db_session: Session, experts_repo: ExpertsRepo, seed_data):
...
def test_list_with_counts_status_filter(db_session: Session, experts_repo: ExpertsRepo, seed_data):
    ...
    results = experts_repo.list_with_counts(db_session, status=[ExpertStatus.active, ExpertStatus.draft])
...
def test_list_with_counts_combined_filters(db_session: Session, experts_repo: ExpertsRepo, seed_data):
...
def test_list_with_counts_prompt_truncation(db_session: Session, experts_repo: ExpertsRepo, seed_data):
...
def test_list_with_counts_returns_expert_list_items(db_session: Session, experts_repo: ExpertsRepo, seed_data):
...
def test_get_with_expanded_existing_expert(db_session: Session, experts_repo: ExpertsRepo, seed_data):
...
def test_get_with_expanded_expert_with_no_links(db_session: Session, experts_repo: ExpertsRepo, seed_data):
...
def test_get_with_expanded_nonexistent_expert(db_session: Session, experts_repo: ExpertsRepo):
    """Test get_with_expanded returns None for nonexistent expert"""
    result = experts_repo.get_with_expanded(db_session, 99999)
    assert result is None


def test_get_with_expanded_workflow_and_service_structure(db_session: Session, experts_repo: ExpertsRepo, seed_data):
